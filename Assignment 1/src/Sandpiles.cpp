...
void spread(Grid<int>& world, int row, int col){
    int val = world[row][col];
    if(val < 4) return;

    world.set(row,col,world[row][col]-4);
...
void dropSandOn(Grid<int>& world, int row, int col) {
    ...
    world.set(row,col,world[row][col]+1);

    while(world[row][col] >= 4){
        spread(world,row,col);
    }
}
